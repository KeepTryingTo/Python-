...
def Canvas_(root):
    ...
    # 创建标签
    label1 = tkinter.Label(root, text='选择的噪声图', font=('黑体', 13), width=30, height=1)
    # `anchor=nw`则是把图片的左上角作为锚定点
    label1.place(x=15, y=20, anchor='nw')
    # 创建标签
    label2 = tkinter.Label(root, text='对应去噪声图', font=('黑体', 13), width=25, height=1)
    # `anchor=nw`则是把图片的左上角作为锚定点
    label2.place(x=280, y=20, anchor='nw')

    # 创建标签
    label3 = tkinter.Label(root, text='原始噪声图', font=('黑体', 13), width=25, height=1)
    # `anchor=nw`则是把图片的左上角作为锚定点
...
def generagedImage(model):
    ...
    input_image = config.transform_only_input(image=inputImg)["image"]
    ...
    cv2.imwrite(f"label.png",cv2.imread(imagePath["filename"]))
    print('inputImg.type: {}'.format(type(inputImg)))
...
def ButtonImage(model1,root):
    # root.withdraw()  # 仅显示对话框，隐藏主窗口
    btn_gen0 = tkinter.Button(root, text='选择高斯噪声图', font=('黑体', 14), height=1, width=20,
                              command = lambda : selectFilename(root))
    btn_gen0.place(x=20, y=355)

    btn_gen1 = tkinter.Button(root, text='生成去噪图', font=('黑体', 14), height=1, width=20,
...
if __name__ == '__main__':
    ...
    model1 = Generator(in_channles=3,features=32)
