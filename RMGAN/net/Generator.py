...
class Generator(torch.nn.Module):
    def __init__(self,in_channles=3,features=32):
        ...
        self.bottleneck = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=features*8,out_channels=features*8,kernel_size=(3,3),
                            stride=(2,2),padding=(1,1),padding_mode='reflect'),
            torch.nn.ReLU()
        )

        self.up1 = Block(in_channels=features * 8, out_channels=features * 8, down=False, act="relu", use_dropout=False)
        self.up2 = Block(in_channels=features * 8 * 2, out_channels=features * 4, down=False, act="relu", use_dropout=False)
        self.up3 = Block(in_channels=features * 4 * 2, out_channels=features * 2, down=False, act="relu", use_dropout=False)
        self.up4 = Block(in_channels=features * 2 * 2, out_channels=features  , down=False, act="relu", use_dropout=False)
    ...
    def forward(self,x):
        ...
        final_up = self.final_up(torch.cat([u4,d1],dim=1))
        return final_up


if __name__ == '__main__':
    gen = Generator(in_channles=3,features=32)
    x = torch.randn(size = (1,3,96,96))
    print(gen(x).shape)
    summary(gen,input_size=(1,3,96,96))
